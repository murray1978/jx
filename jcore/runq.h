...
static void domainrunq_append(DomainDesc * domain);
static DomainDesc *domainrunq_remove();

static int threadrunq_append(DomainDesc * domain, ThreadDesc * thread);
static ThreadDesc *threadrunq_remove(DomainDesc * domain);
...
static void Sched_block(u4_t state);
